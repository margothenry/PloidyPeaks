...
devtools::install_github("margothenry/PloidyPeaks")
library(PloidyPeaks)
...
knitr::include_graphics(here("vignette/images/gating.png"))
```  
...
```{r, echo=FALSE, fig.pos = "H", fig.align="center"}
knitr::include_graphics(here("vignette/images/V2_gated1.png"))
```  
...
```{r, echo=FALSE, fig.pos = "H", fig.align="center"}
knitr::include_graphics(here("vignette/images/V2_gated2.png"))
``` 
...
##### Warnings:  
The following warning appears when you run the gating functions with save_plot = TRUE. No need to worry! This warning is letting us know that they are using different coordinates for the graphs then the suggested coordinates. 

`Warnings: Coordinate system already present. Adding new coordinate system, which will replace the existing one.`  
...
```{r}
 flowLineGraph(
  flowDir = here("vignette/data/gated_data"),
  flowControl = "V2",
  flowSamples = c("V3","V5", "V7"),
  xVariable = "FL1-A"
)
```  
...
flowLineGraph(
  flowDir = here("vignette/data/gated_data"),
  flowControl = "V23",
  flowSamples = c("V11", "V9", "V20", "V21", "V22", "V24", "V18"),
  xVariable = "FL1-A"
)
``` 
...
```{r, echo=FALSE, fig.pos = "H", fig.align="center"}
knitr::include_graphics(here("vignette/images/smoothing.png"))
```   
...
```{r, echo=FALSE, fig.pos = "H", fig.align="center"}
knitr::include_graphics(here("vignette/images/findCluster.png"))
```   
...
Once all sub population peaks are identified, the proportion of cells that are being used is calculated (Figure 8). If the proportion of cells used (cellProp) is under our threshold of 87%, that sample will be flagged for further analysis. Currently, the threshold has been determined by looking at the cellProp from known control populations composed of known *Candida albicans* single populations. But there is an option for researchers to infer an appropriate threshold from their data.  

```{r, echo=FALSE, fig.pos = "H", fig.align="center"}
knitr::include_graphics(here("vignette/images/findPairs_cellProp.png"))
```  
...
```{r, echo=FALSE, fig.pos = "H", fig.align="center"}
knitr::include_graphics(here("vignette/images/doublet.png"))
```   
...
Therefore, the two curves that can be applied are:  

For populations without doublets:  
$$
F(x) = F_1(x) + F_2(x) + F_{s1}(x)
$$  

For populations with doublets:    
$$
...
rectGateFlowFrame(
  raw_dir = here("vignette/data/raw"),
  flow_name = "V2",
  x_variable = "FL1-A",
  y_variable = "SSC-A",
  x_min_value = 30000,
